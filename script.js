d3.json("data/forest.json").then(data => {
    ...
    bars.on("mouseover", function(e, d) {
          d3.select(this)
            .attr("fill", d => d3.color(color(d3.select(this.parentNode).datum().key)).brighter(1)); // Highlight by making the color brighter
          tooltip.html(`Year: ${d.data.year}<br>${d3.select(this.parentNode).datum().key}: ${d[1] - d[0]}`)
       ...
       .on("mouseout", function() {
          d3.select(this)
            .attr("fill", d => color(d3.select(this.parentNode).datum().key)); // Revert to original color
